...
from hrap.utils.impulse import impulse
...
def load_propellant_data(propellant_name="Paraffin"):
    ...
    propellant_path = project_root / "hrap" / "propellants" / f"{propellant_name}.json"
...
def run_paraffin_integration_test():
    ...
    """
    print("Running HRAP integration test with Paraffin propellant...")
    ...
    plt.suptitle(f"HRAP Simulation Results - {s['prop_nm']} Propellant")
    plt.tight_layout()
    plt.subplots_adjust(top=0.92)
    
    # Save the figure
    plot_path = project_root / "plots" / "hrap_integration_test_results.png"
